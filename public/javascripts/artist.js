function load_artist(url)
{
   $.get(url, function(data){
      ...
      elem.innerHTML = " <a href='' onclick=''> <span class='glyphicon glyphicon-pencil'></span> Rename Artist </a>";
      ...
      for(var i in songs)
      {
         ...
         cell1.innerHTML = "<a href='' onclick='load_album("+JSON.stringify('/album/'+song_parsed.album)+"); return false;'>"+song.album+"</a>";
      ...
      for(i in albums)
      {
         ...
         elem = document.getElementById("artist_single_albums");
         var album_json = replaceAll(song.title,"%27", "'");
         elem.innerHTML += "<a href='' onclick='load_album("+JSON.stringify('/album/'+album_json)+"); return false;' > <div title='"+album+"' class='album col-md-5ths'> <div class='song_image'> <img onerror="+x+" src='"+album_art+"'> </img> </div> <div class='ellip_name'> "+album+" </div> </div> </a> ";
